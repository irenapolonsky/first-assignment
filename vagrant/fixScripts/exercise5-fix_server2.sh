...
cat /vagrant/fixScripts/server2_id_rsa_pub.txt  > /home/vagrant/.ssh/id_rsa.pub
...
chmod 640 /home/vagrant/.ssh/id_rsa.pub
...
status=$?
if [ ${status} -eq "0" ];
        then echo "*****public key of server 1 is authorized on Server2*****"
        else
        echo "********something went wrong status $? ******"
fi
